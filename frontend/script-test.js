...
function previewRecording() {
  ...
  // Play stadium noise during preview
...
function resetRecording() {
  ...
  ['recording-stadium-noise', 'preview-stadium-noise'].forEach(id => {
...
function shareOnSocial(platform) {
  ...
  const shareText = 'Check out my voice-over commentary on this Rajasthan Royals moment! Try yours at \n' + window.location.origin + ' #RajasthanRoyals #HallaBol #VoiceOver';
  
  let shareWindowUrl;
  
  switch (platform) {
      case 'twitter':
          // Twitter/X sharing - this works without any API keys
...
function setupSynchronizedPlayback(videoPath, audioPath) {
  ...
  // Create audio element for the recording (voice over)
  ...
  // Create stadium noise audio element - use the working path from your logs
  const stadiumNoise = document.createElement('audio');
  stadiumNoise.src = `${API_BASE_URL}/stadium-noise.mp3`; // Use the path that works
  stadiumNoise.id = 'stadium-noise';
  stadiumNoise.loop = true; // Make it loop continuously
  stadiumNoise.volume = 0.3; // Set to 30% volume so it doesn't overpower the commentary
  document.body.appendChild(stadiumNoise);
  
  // Synchronize video and audio playback
  finalVideo.onplay = () => {
      // Start both audio elements when video plays
  ...
  finalVideo.onpause = () => {
      // Pause both audio elements when video pauses
      voiceOverAudio.pause();
      stadiumNoise.pause();
  };
  
  finalVideo.onseeked = () => {
      // Keep voice-over in sync when seeking
      voiceOverAudio.currentTime = finalVideo.currentTime;
  };
